...
void textured_mesh_exercise()
{
	...
	auto texture{ std::make_shared<PatternFile>(".\\assets\\crab.ppm") };
	// Neutral gray material
	auto phong = std::dynamic_pointer_cast<Phong>(mesh->material);
	if (phong) {
		phong->pattern = texture;
	...
	ppm.write_to_file("./x64/Release/crab_textured.ppm");
}